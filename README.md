...
- Frontend: an interactive web page with `AJAX` technology implemented with `HTML`, `CSS` and `JavaScript`. The Event Recommendation Website realizes three main functions:
   ...
   * **Favorite** events they like and also delete events they donâ€™t like anymore
...
![mysql](https://raw.githubusercontent.com/apollotong/TicketMasterPlus/tree/master/doc/mysql.png)
...
- Design pattern
   ...
   * **Factory pattern**: `ExternalAPIFactory.java`, `DBConnectionFactory.java`
      * `ExternalAPIFactory.java`: support multiple function like recommendation of event, restaurant, news, jobsâ€¦ just link to different public API like TicketMasterAPI. Improve extension ability.
